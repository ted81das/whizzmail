...
class SendCampaignsCommand extends ConsoleCommand
{
     ...
     * @since 2.1.17
     */
    protected const CAMPAIGNS_SELECTION_QUERY_STATUSES = [Campaign::STATUS_SENDING, Campaign::STATUS_PENDING_SENDING];
    protected const CAMPAIGNS_SELECTION_QUERY_TYPES = [Campaign::TYPE_REGULAR, Campaign::TYPE_AUTORESPONDER];
    ...
    protected function process()
    {
        ...
        $statuses = self::CAMPAIGNS_SELECTION_QUERY_STATUSES;
        $types    = self::CAMPAIGNS_SELECTION_QUERY_TYPES;
        ...
        $criteria->addCondition('(
			(t.send_between_start IS NULL OR t.send_between_interval IS NULL) 
		    OR 
		    (
                DATE_FORMAT(NOW(), "%Y-%m-%d %H:%i:00") 
                BETWEEN 
                    DATE_FORMAT(NOW(), CONCAT("%Y-%m-%d", " ", t.send_between_start)) 
                AND 
                    DATE_ADD(DATE_FORMAT(NOW(), CONCAT("%Y-%m-%d", " ", t.send_between_start)), INTERVAL t.send_between_interval HOUR)
		    )
		)');
        ...
        // offer a chance to alter the criteria.
        ...
        foreach ($campaigns as $campaign) {
            // since 2.1.17
            if (!in_array($campaign->status, self::CAMPAIGNS_SELECTION_QUERY_STATUSES)) {
                continue;
            }

            // since 2.2.12
            // see https://github.com/onetwist-software/mailwizz/issues/860
            if (!mutex()->acquire($campaign->getSendCampaignFirstLevelMutexKey())) {
                $this->stdout(sprintf(
                    '[!!!] Cannot acquire mutex lock for campaign %d which means, it is already running in a different process!',
                    $campaign->campaign_id
                ));
                continue;
            }
            mutex()->release($campaign->getSendCampaignFirstLevelMutexKey());
            //
        ...
        foreach ($campaigns as $campaign) {
            ...
            $campaignDeliveryLogSuccessCount = $campaign->getCampaignDeliveryLogModel()->countByAttributes([
    ...
    protected function sendCampaignsPreChecksStep1(int $campaignId, string $hashKey, int $workerNumber = 0)
    {
        ...
        // since 2.2.17 added filter hook
        $mutexHashTTL = (int)hooks()->applyFilters('console_command_send_campaigns_pre_checks_step1_mutex_hash_ttl', 120);
        ...
        // since 2.2.17
        $pcdMutexHashTTL = (int)hooks()->applyFilters('console_command_send_campaigns_pre_checks_step1_populate_customer_data_mutex_hash_ttl', 240);

        $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Populating customer data...', $workerNumber, $campaignId));
        if (mutex()->acquire($hashKey, $pcdMutexHashTTL)) {
            $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Checking the cache data...', $workerNumber, $campaignId));
            $cachedData = cache()->get($hashKey);
            if (empty($cachedData) || !is_array($cachedData)) {
                $this->stdout(sprintf('[Worker %d] [ERROR] Campaign id %d pre-checks: Unable to fetch cached data on line %d!', $workerNumber, $campaignId, __LINE__));
                mutex()->release($hashKey);
            ...
            $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: The cache data is correct...', $workerNumber, $campaignId));
            ...
            if (!isset($cachedData['customerData'][$campaign->customer_id])) {
                ...
                if ($quotaTotal > -1) {
                    $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Start counting usage from quota mark...', $workerNumber, $campaignId));
                    $quotaUsage = (int)$customer->countUsageFromQuotaMark();
                    $quotaLeft  = $quotaTotal - $quotaUsage;
                    $quotaLeft  = (int)max($quotaLeft, 0);
                    $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Finished counting usage from quota mark...', $workerNumber, $campaignId));
                }

                // 1.3.9.7
                if ($customer->getCanHaveHourlyQuota()) {
                    $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Start getting hourly quota left...', $workerNumber, $campaignId));
                    $hourlyQuotaLeft = $customer->getHourlyQuotaLeft();
                    if ($hourlyQuotaLeft <= $quotaLeft) {
                        $quotaLeft = $hourlyQuotaLeft;
                        $quotaLeft = (int)max($quotaLeft, 0);
                    }
                    $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Finished getting hourly quota left...', $workerNumber, $campaignId));
                }

                $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Start counting subscribers...', $workerNumber, $campaignId));
                ...
                $this->stdout(sprintf('[Worker %d] Campaign id %d pre-checks: Finished counting subscribers in %.5f seconds!', $workerNumber, $campaignId, $elapsedTime));
            ...
            foreach ($campaignChunks as $index => $cids) {
                ...
                foreach ($cids as $cid) {
                    ...
                    if (!$pid) {
                        ...
                        if (!empty($campaign) && mutex()->acquire($campaign->getSendCampaignFirstLevelMutexKey())) {

                            // 1.5.3
                            try {
                                $this->sendCampaignStep1($campaign, $index+1);
                            } catch (Exception $e) {
                                $this->stdout(__LINE__ . ': ' . $e->getMessage());
                                Yii::log($e->getMessage(), CLogger::LEVEL_ERROR);
        ...
        if (!$handled) {
            foreach ($campaignIds as $campaignId) {
                ...
                if (!empty($campaign) && mutex()->acquire($campaign->getSendCampaignFirstLevelMutexKey())) {
                    ...
                    // 1.5.3
                    try {
                        $this->sendCampaignStep1($campaign);
                    } catch (Exception $e) {
                        $this->stdout(__LINE__ . ': ' . $e->getMessage());
                        Yii::log($e->getMessage(), CLogger::LEVEL_ERROR);
     ...
     * @throws Throwable
     */
    protected function sendCampaignStep1(Campaign $campaign, int $workerNumber = 0)
    {
        $this->stdout(sprintf('Campaign Worker #%d looking into the campaign with ID: %d', $workerNumber, $campaign->campaign_id));

        // since 1.3.7.3
        hooks()->doAction('console_command_send_campaigns_send_campaign_step1_start', $campaign);

        if (!in_array($campaign->status, self::CAMPAIGNS_SELECTION_QUERY_STATUSES)) {
            $this->stdout(sprintf('The campaign with ID: %d is not ready for processing.', $campaign->campaign_id));
            return 1;
        }
    ...
    protected function sendCampaignStep2(array $params = [])
    {
        ...
        $maxSubscribers = intval($this->_customerData[$params['customer']->customer_id]['campaigns'][$params['campaign']->campaign_id] ?? 0);
        ...
        if ($this->getCanUsePcntl() && ($subscriberBatchesInParallel = $this->getSubscriberBatchesInParallel()) > 1) {
            ...
            $initialParamsLimit    = (int)$params['limit'];
            $newSubscriberBatchesInParallel = 0;
            for ($i = 0; $i < $subscriberBatchesInParallel; ++$i) {
                if ($maxSubscribers <= (int)$params['limit']) {
                    $params['limit'] = (int)$maxSubscribers;
                }
                $maxSubscribers -= (int)$params['limit'];
                $maxSubscribers  = (int)max($maxSubscribers, 0);
                $params['limit'] = (int)max((int)$params['limit'], 0);
            ...
            $fsCounterKey = self::class . '::findSubscribersSyncLock::' . $params['campaign']->campaign_id;
            ...
            for ($i = 0; $i < $subscriberBatchesInParallel; ++$i) {
                ...
                $maxSubscribers -= (int)$params['limit'];
                $maxSubscribers  = (int)max($maxSubscribers, 0);
                $params['limit'] = (int)max($params['limit'], 0);
                ...
                $offset                            = intval($params['offset'] + $params['limit']);
                //
                ...
                if (!$pid) {
                    ...
                    // 2.2.12
                    $mutexKey = $campaign->getSendCampaignSecondLevelMutexKey((int)$params['offset'], (int)$params['limit']);
    ...
    protected function sendCampaignStep3(array $params = [])
    {
        ...
        if ($this->getCanUsePcntl() && ($subscriberBatchesInParallel = $this->getSubscriberBatchesInParallel()) > 1) {
            ...
            $startTime      = microtime(true);
            $fsCounterKey   = self::class . '::findSubscribersSyncLock::' . $campaign->campaign_id;

            // each process must decrement its own increment, so just once!
            ...
            while (true) {
                if (!mutex()->acquire($fsCounterKey)) { // @phpstan-ignore-line
                    continue;
                }
                $fsCounter = (int)cache()->get($fsCounterKey);
                if ($fsCounter <= 0) {
    ...
    protected function processSubscribersLoop(array $params = [])
    {
        ...
        foreach ($subscribers as $index => $subscriber) {
            ...
            if ($server->getIsOverQuota()) {
                ...
                // since 2.4.0
                $currentServerHasSecondQuota = $server->getCanHaveSecondQuota();
                $newServer = DeliveryServer::pickServer($currentServerId, $campaign, $dsParams);
                if ($newServer === null && $currentServerHasSecondQuota) {
                    $this->stdout('We were not able to select a new delivery server and since the current delivery server has the second_quota set, so we are going to retry it...');
                    sleep(2);
                    // @phpstan-ignore-next-line
                    if (!$server->getIsOverQuota()) {
                        $newServer = $server;
            ...
            if (!empty($listUnsubscribeHeaderEmail)) {
                ...
                $mailToUnsubscribeHeader    = sprintf(', <mailto:%s?subject=%s&body=%s>', $listUnsubscribeHeaderEmail, rawurlencode($_subject), rawurlencode($_body));
            ...
            if (!empty($headers)) {
                ...
                foreach ($headers as $header) {
                    ...
                    // since 2.2.3
                    if (strtolower((string)$header['name']) == 'x-force-return-path' && stripos($header['value'], '@') !== false) {
                        $emailParts      = (array)explode('@', $header['value']);
                        $domainName      = (string)array_pop($emailParts);
                        $header['value'] = trim(sprintf('%s@%s', implode('=', $emailParts), $domainName), ' =');
            ...
            // since 1.4.2
            if ($sent) {
                $this->handleCampaignSentActionSubscriberField($campaign, $subscriber, $server);
            ...
            if (!empty($server->pause_after_send)) {
                ...
                // close the external connections otherwise will time out.
                if ($sleepSeconds >= 30) {
                    $this->setExternalConnectionsActive(false);
                }
        ...
        // since 1.3.6.3 - it's not 100% bulletproof but should be fine
    ...
    {
        // since 2.3.8
        if (hooks()->hasFilters('console_send_campaigns_command_count_subscribers')) {
            return (int)hooks()->applyFilters('console_send_campaigns_command_count_subscribers', 0, $campaign);
        }
    ...
    protected function findSubscribers(int $offset, int $limit, Campaign $campaign): array
    {
        ...
        // since 2.3.8
        if (hooks()->hasFilters('console_send_campaigns_command_find_subscribers')) {
            return (array)hooks()->applyFilters('console_send_campaigns_command_find_subscribers', [], $offset, $limit, $campaign);
        }
    ...
    protected function prepareEmail(ListSubscriber $subscriber, DeliveryServer $server, Campaign $campaign)
    {
        ...
        if (!$onlyPlainText) {
            ...
            if (CampaignHelper::hasCountdownTag($emailContent)) {
                ...
                $this->stdout(sprintf('Parsed and loaded the email content countdown images in %.5f seconds.', round(microtime(true) - $start, 5)));
            }
            //
        ...
        if (!empty($campaign->option) && $campaign->option->plain_text_email == CampaignOption::TEXT_YES) {
            if ($campaign->template->auto_plain_text === CampaignTemplate::TEXT_YES /* && empty($campaign->template->plain_text)*/) {
                ...
                $ignoreLink = CampaignHelper::htmlToText(CampaignHelper::getClickIgnoreBeacon($campaign, $subscriber));
                $emailPlainText = str_ireplace($ignoreLink, '', $emailPlainText);
            }
        ...
        if (CampaignHelper::hasRemoteContentTag($emailSubject)) {
            $start = microtime(true);
            $emailSubject = CampaignHelper::fetchContentForRemoteContentTag($emailSubject, $campaign, $subscriber, $server);
    ...
    protected function checkCustomersQuotaLimits()
    {
        ...
        foreach ($this->_customerData as $cdata) {
            ...
            // since 2.2.12 - Fetch the total again
            $quotaTotal = (int)$customer->getGroupOption('sending.quota', -1);
            ...
            // since 2.2.12 - Get the fresh usage quota
            $quotaUsage = (int)$customer->countUsageFromQuotaMark();
     ...
     * @return $this
     */
    protected function handleCampaignSentActionSubscriberField(Campaign $campaign, ListSubscriber $subscriber, DeliveryServer $server): self
    {
     ...
     * @return $this
     */
    protected function handleCampaignSentActionSubscriber(Campaign $campaign, ListSubscriber $subscriber): self
    {
        static $sentActionModels = [];
